...
> **✅ 전 단계 완료 (2026-08-31 23:24 KST).** Phase 0(마이그레이션·로컬 실측),
> Phase 1(백업 3종), Phase 2(컷오버) 모두 종료. 운영은 Tomcat 10.0.20 / JDK 17 /
> Boot 3.1.12로 전환됐고 Metaspace 48.5MB/128MB(37%), 힙도 128MB로 확대됐다.
> 컷오버 실측 기록은 §10 참조. 관리자 JSP 콜드 경로도 23:27에 실측 완료(+2.1MB,
> §10 말미). 남은 것은 안정화 기간 후 서버 `~/tmp` 백업 정리뿐이다.
...
## 10. 컷오버 실측 기록 (2026-08-31 23:15~23:24 KST)

- **백업(23:16)**: DB 덤프(테이블 80개, gzip 무결성 확인), 파일 tar(WAR·config·
  conf·setenv·로그), 이미지 tar(42MB), 런타임 상태 기록. 로컬
  `~/Documents/sc1hub-backups/20260831-boot3-cutover/`(SHA256SUMS 포함)와
  서버 `~/tmp/` 이중 보관.
- **서버환경 변경(23:19 제출)**: 신청 즉시(1분 내) 적용됐다 — 안내상 "5분 이내".
- **변경이 실제로 한 일**: `tomcat/` 디렉터리를 **통째로 새 Tomcat 10.0.20으로
  교체**(구 WAR·logs·webapps/img 소멸). `config/`·`scripts/`·`backup/`·`~/tmp/`·
  **DB는 그대로 보존**. 새 `server.xml`은 포트 8645 유지. `setenv.sh`는 백지 —
  구 `-Xms16m -Xmx64m`도 함께 사라져서 JAVA_OPTS의 `-Xmx128m`이 처음으로 적용됐다.
- **복원**: 이미지 tar를 새 webapps에 풀어 42MB 복원. 나머지(setenv 재구성,
  스크립트 업로드, WAR 설치)는 deploy.sh가 수행.
- **배포(23:21~23:23)**: 빌드·테스트 통과 → 프리플라이트(Tomcat 10.0/JVM 17
  확인) 통과 → 기동 → 워밍업 → **Metaspace 게이트 47,755KB/131,072KB(36%) →
  48,540KB(37%)** 2회 통과.
- **최종 상태**: `MaxMetaspaceSize=128MB`, `MaxHeapSize=128MB`(기존 64MB에서
  2배), 공개 경로 전부 200, 관리자 API 비인증 403, 복원 이미지 200,
  catalina 에러 0건, MetaspaceUsageLogger 10분 샘플링 가동(37%).
- 로컬 실측(47.0MB)과 운영 실측(47.8MB)이 0.8MB 차이로 일치 — Phase 0의
  로컬 리허설 방법론이 유효함을 확인.
- **관리자 JSP 콜드 경로(23:27)**: 운영자 로그인 후 adminPage →
  aliasDictionary → ops를 첫 접근. **49.2MB(37.5%) → 51.4MB(39.2%), +2.1MB /
  클래스 +169개.** 과거 64MB 시절 이 계단이 남은 여유의 대부분을 삼켰지만,
  지금은 여유 약 78MB의 3%다. WARN·에러 0건. 특기: JDK 17에서는 클래스
  언로드도 실제로 일어난다(57개 관측 — JDK 8 운영에서는 Unloaded가 항상 0이었다).
