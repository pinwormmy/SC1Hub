...
일반 쓰기는 2026-09-07 재개됐고, 같은 날 비회원 쓰기도 내용 검사·자동 제재를 붙여 다시 열었다. 이 문서는 (1) 상시 자동으로 동작하는 방어, (2) 징후를 어떻게 보고,
...
| 계층 | 규칙 | 위치 |
|---|---|---|
| 비회원 쓰기 | 허용(스위치로 즉시 차단 가능). IP당 전체 10회/분, 글 3회/10분, 댓글 10회/10분, 이미지 5회/10분, 전체 60회/분 | `PublicWriteSecurityInterceptor` |
...
1. **즉시 얼리기**: 비회원만 문제면 먼저 "비회원 쓰기 차단"(`POST /api/admin/security/guest-writes {"enabled":false}`).
   회원 계정까지 동원됐으면 보안 운영 카드의 "회원 쓰기 차단(긴급)" 또는
...
- 스위치는 **재시작하면 `application.properties` 기본값**(회원·비회원 쓰기 모두 허용)으로 돌아간다.
  장기간 차단이 필요하면 프로퍼티를 바꿔 배포한다.
- 잘못 걸린 제재는 활성 제재 목록에서 해제한다. 자동 제재는 `created_by=auto` 로 구분되고, 사유에
  `공격 패턴 감지: <규칙명>`(script-tag, event-handler, js-uri, foreign-iframe, sql-injection, link-flood 등)이 남는다.
  정상 이용자가 `<script>` 같은 문자열을 그대로 적어 24시간 제재를 받았다면 해제하고, 반복되면 규칙을
  `AttackContentDetector` 에서 조정한다. 운영자 계정으로 시험할 때는 등급 3 이라 제재가 걸리지 않는다.
