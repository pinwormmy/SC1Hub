...
### 비회원 쓰기 재개와 공격 내용 자동 제재 (5차 반영)

- 비회원 쓰기(글·댓글·채팅·한줄 공략·이미지)를 기본 허용으로 되돌렸다(`sc1hub.security.guest-writes-enabled=true`,
  관리자 스위치로 즉시 차단 가능). 지원·영상링크·홍보 게시판은 그대로 열려 있다.
- 비회원 IP별 한도를 추가했다: 글 3회/10분, 댓글 10회/10분, 이미지 업로드 5회/10분(기존 전체 10회/분, 60회/분 유지).
- `AttackContentDetector` 가 저장 전 내용에서 공격 의도를 찾는다. HIGH(스크립트 태그, 이벤트 핸들러, `javascript:`,
  유튜브 외 iframe, 속성 달린 svg/object/embed/meta/base/link/form, `data:text/html`, SQL 주입 구문, `${}`·`<% %>`·JNDI,
  경로 조작, 명령 실행 구문)는 거부와 동시에 **즉시 24시간 제재**(회원 뮤트, 비회원은 프록시 헤더로 확인된 공인 IP 차단).
  MEDIUM(링크 도배, 관리자 사칭+외부 링크+인증 유도, AI 에게 관리자·DB 자격증명 요구)은 거부 후 스트라이크 5회분 누적.
  적용 지점: 글 등록·수정, 댓글, 채팅, AI 질문, 한줄 공략, 가입 필드. 등급 3 관리자는 제재 대상에서 제외한다.
  정규식은 되돌림 폭발이 없는 형태로만 쓰고 검사 길이를 20,000자로 제한한다. 정화기(`PostContentSanitizer`)는 그대로
  동작하므로 감지를 빠져나간 마크업도 저장 전에 제거된다.
- 오탐 처리: 제재 사유에 규칙명이 남고 관리자 화면에서 해제할 수 있다. 위키 붙여넣기에 흔한 `{{ }}` 와 속성 없는
  단순 태그 문자열은 규칙에서 제외했다.
...
- **한줄 공략 기능 제거 (2026-09-07, 운영자 지시)**: 181건 중 166건이 9월 도배 기간의 비회원 글이었고 이용률도 낮아
  기능을 통째로 걷어냈다. 컨트롤러·서비스·매퍼·화면·CSS, 사이드바 21번·상단 메뉴·첫 화면 버튼·최근 글 집계·사이트맵·
  SEO 정책·AI 검색 색인(RAG 소스, factBoards)·쓰기 관문 경로를 제거했고, `sql/20260907_drop_one_line_strategy.sql` 로
  본문·분류·추천 테이블과 예전에 코드만 지워졌던 AI 초안 테이블 2개까지 정리했다(총 5개, Tomcat 정지 후 적용).
  삭제 전 전체 덤프는 미추적 `.local-data/board-cleanup-20260907/`. 배포 후 RAG 인덱스를 재구축해 옛 청크를 걷어낸다.
- **문의게시판(supportboard) 정리 (2026-09-07, 운영자 지시)**: 글 3건(마지막 2024-12-14)·댓글 8건뿐이고 어떤 메뉴에도
  연결되지 않은 방치 게시판이라 통째로 정리했다. 코드(게시판 목록 정규화, 관리자 예외 경로, 첫 화면 최근 글 집계,
  AI 색인 제외 목록)에서 제거하고 `sql/20260907_drop_supportboard.sql` 로 `board_list` 행과 4개 테이블
  (본문·댓글·추천·조회)을 정리했다. 정리 전 전체 덤프는 미추적 `.local-data/board-cleanup-20260907/` 에 보존.
  같이 정리: 테이블도 목록 행도 없던 유령 게시판 이름(freeboard·beginnerboard·userguideboard)을 코드에서 제거해
  해당 주소가 500 대신 다른 미존재 게시판과 같은 400 으로 응답하도록 했고, 이용 안내 페이지의 죽은 링크를 고쳤다.
  운영 확인: `/boards/supportboard`·`/boards/freeboard` 400, 첫 화면·이용 안내·영상자료실·홍보 게시판 200, board_list 15행.
