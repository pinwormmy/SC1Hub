...
3. **사건 당일 대량 가입 계정**: 증거 스냅샷 기준 회원 530명 중 472명이 2026-09-06 하루에 가입했다
   (평소 월 1명 수준). 재감사가 집계한 실행 태그 패턴 회원 2건도 모두 이 계정군에 속한다(등급 1,
   `<script` 별명). 출력 이스케이프로 실행은 막혀 있다. 삭제·권한 박탈(등급 0) 등 처리 방침은
   운영자 판단이며, 일반 쓰기 재개 전에 결정해야 한다(원본 식별자·별명은 증거 스냅샷에 보존됨).
4. **일반 쓰기 재개**: 위 항목과 회귀 검증까지 마친 뒤에 `public-writes-enabled` 해제를 판단한다.
