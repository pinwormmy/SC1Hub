...
- 하루치 최대 3건을 분류별로 한 건씩 Gemini Interactions API에 요청합니다. 응답의 검색 인용을 해당 초안 하나에 명확히 연결하며, 서버가 본문 12~96자와 근거 설명 5~72자를 다시 검증합니다.
- 각 초안에는 Gemini가 실제 Google Search에서 인용한 외부 출처 링크가 반드시 저장됩니다.
- 검수 대기 초안이 3건이면 새 생성을 멈춰 대기열이 쌓이지 않습니다.
- 성공한 초안은 즉시 검수 대기로 저장하므로 뒤의 분류가 실패해도 유지됩니다. 다음 예약·수동 실행은 남은 슬롯만 보충하며, 초안 하나당 호출 한 번으로 하루 총 3회를 넘지 않습니다.
...
공유 기본값은 Google Search grounding과 구조화 출력을 함께 지원하는 `gemini-3.6-flash`의 `minimal` 사고 수준, 호출당 600 출력 토큰, 하루 3건, 대기 3건이며 매시 5분에 당일 누락분을 확인합니다. 긴 외부 URL과 제목은 모델 JSON에 중복 생성하지 않고 Gemini의 네이티브 인용 메타데이터에서 저장하며, 내부 원문 발췌 기본값은 분류별 글당 360자로 제한해 비용을 억제합니다. 각 분류를 별도 호출로 생성하고 호출당 Google Search를 정확히 한 번만 사용하며, DB의 일일 3회 호출 상한과 `(generation_date, slot_no)` 유니크 키가 과금·중복 저장을 제한합니다. 분류별 요청은 공통 프롬프트를 최대 세 번 보내므로 단일 배치보다 입력 토큰이 소폭 늘지만, 검색 횟수는 하루 최대 3회로 같고 인용과 초안의 대응이 명확해집니다. 수동 생성도 관리자 검수 화면에서 같은 제한 안에서만 실행됩니다.
