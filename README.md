...
- 하루치 최대 3건을 Gemini Interactions API 한 번에 요청합니다. 3건이 1,200 출력 토큰 안에 안정적으로 들어오도록 본문·근거 설명 길이를 프롬프트에서 제한하고, 응답 후 서버가 본문 12~96자와 근거 설명 5~72자를 다시 검증합니다.
...
공유 기본값은 Google Search grounding과 구조화 출력을 함께 지원하는 `gemini-3.6-flash`의 `minimal` 사고 수준, 1,200 출력 토큰, 하루 3건, 대기 3건이며 매시 5분에 당일 누락분을 확인합니다. 긴 외부 URL과 제목은 모델 JSON에 중복 생성하지 않고 Gemini의 네이티브 인용 메타데이터에서 저장하며, 내부 원문 발췌 기본값은 분류별 글당 360자로 제한해 비용을 억제합니다. 요청할 때는 세 분류를 한 배치로 묶되 분류마다 검색 질의를 정확히 한 번만 사용하고, DB의 일일 호출 상한과 `(generation_date, slot_no)` 유니크 키가 과금·중복 저장을 제한합니다. 수동 생성도 관리자 검수 화면에서 같은 제한 안에서만 실행됩니다.
