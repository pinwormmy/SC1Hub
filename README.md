...
AI 한줄 공략은 공개 글을 직접 작성하지 않습니다. 사이트의 종족전·팁·팀플 공략 게시글만 근거로 비공개 초안을 만들고, 관리자가 `/adminPage/strategy-tips/ai`에서 연결된 원문을 확인해 편집·승인한 경우에만 공개됩니다. Gemini의 Google Search·URL Context 도구와 모델의 외부 지식은 이 기능에서 사용하지 않습니다.
...
- 모델이 선택한 내부 게시글의 원문 구절을 근거로 그대로 반환하게 하고, 초안과 승인 편집문 모두에 근거 구절 전체가 그대로 남아 있어야 통과시킵니다. 서버는 실제 발췌문 포함 여부·카테고리·게시글 연결·숫자·본문 12~96자·중복·단정·시의성 표현도 다시 검증합니다.
- 내부 근거 글이 없는 분류는 외부 자료로 채우지 않고, 내부 글이 있는 다음 분류로 건너뜁니다.
