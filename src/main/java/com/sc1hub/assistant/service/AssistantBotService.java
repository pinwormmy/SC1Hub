...
public class AssistantBotService {
    ...
    private final AssistantBotPublishFormatter publishFormatter;
    ...
    AssistantBotService(AssistantBotProperties botProperties,
        ...
        this.publishFormatter = new AssistantBotPublishFormatter();
    }
    ...
    private BoardDTO buildPostForPublish(PersonaProperties persona, JsonNode result) {
        ...
        post.setTitle(publishFormatter.safeTitleForPublish(persona, textOrNull(result.path("post").path("title"))));
    ...
    private String normalizeChatBody(PersonaProperties persona, String body) {
        ...
        String normalized = body.trim()
                ...
                .trim();
        if (isRepetitiveByDesignPersona(persona)) {
            normalized = publishFormatter.normalizeMeowPublishText(normalized)
                    .replace('\n', ' ')
                    .replaceAll("\\s+", " ")
                    .trim();
        }
        if (isSingleSentenceOnlyPersona(persona)) {
            normalized = publishFormatter.firstSentenceOnly(normalized);
        }
    ...
    private JsonNode parseJson(String raw) {
        return responseParser.parseObject(raw);
    }
    ...
    private String toHtmlBody(PersonaProperties persona, String body) {
        return publishFormatter.toHtmlBody(persona, body);
    }

    private String safeCommentForPublish(PersonaProperties persona, String body) {
        return publishFormatter.safeCommentForPublish(persona, body);
    }

    private boolean isSingleSentenceOnlyPersona(PersonaProperties persona) {
        return publishFormatter.isSingleSentenceOnlyPersona(persona);
    }
    ...
    private boolean isRepetitiveByDesignPersona(PersonaProperties persona) {
        return publishFormatter.isRepetitiveByDesignPersona(persona);
    }
